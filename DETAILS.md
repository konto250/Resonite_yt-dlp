...
[前回の記事](./README.md)（[X](https://x.com/konto250/status/2089005526151803166)のポスト）では対処法だけを書きました。要点は以下です。
...
このコマンドラインは、自分のログから丸ごとコピーして手元で実行できます。Resonite を起動せずに挙動を確認できるので、切り分けに使えます。
---

## 2. Resonite は映像と音声が1本になっているフォーマットを優先する
...
> **【要検証】映像が出ず音声だけ再生されるという報告について**
>
> 上の書き方だと「映像+音声が1本のものが1つもない場合」がどうなるか分かりません。単独読み込み時の比較ログを見ると、映像+音声のフォーマットが現れる前の時点で、音声のみのフォーマットが最良候補に入っています。
>
> ```
> Codec none + opus             vs none + opus
> Codec avc1.4d400c + none      vs none + none
> Codec avc1.42001E + mp4a.40.2 vs none + mp4a.40.2   ← ここで映像+音声が採用される
> ```
>
> 正確には「映像+音声が1本のものがあればそれを選ぶ。なければ手前の候補が残る」という挙動かもしれません。だとすると、音声のみが選ばれて映像が出ないケースがありうることになります。
>
> ただし手元の22件のログでは `Best Format: Video: none` は一度も出ていないので、確認できていません。
>
> 同じ症状の方は、ログの `Best Format:` の行を見てください。
...
自分の環境がどちらか確認するには、Resonite のインストールフォルダ内の `Logs` から最新のログを開き、`Best Format:` と `Audio tracks:` を検索してください。場所が分からない場合は、ファイル名が `<PC名>_-_<バージョン>_-_<日時>.log` の形式なので、これで検索すると見つかります。

なお、ワールドやアセットによっては複数の動画が同時に読み込まれます。その場合 `Loading video from:` と `Audio tracks:` の対応が付かないので、何本中何本が成功したかを数える程度しか分かりません。
...
念のため書いておくと、これは Firefox の方が安全という話ではありません。Chrome は 127 以降、Cookie の復号鍵をブラウザ自身に紐づける仕組みを導入しており、外部ツールから読めない場合があります。Firefox が有利なのは、外部ツールから読めるかどうかという点だけです。
...
ただし途中で Claude は誤った結論を何度か出しています。時系列の観測から因果を読もうとして間違えていました。最終的には条件を1つずつ変える対照実験に組み直して確定させています。5章の数字はその結果です。