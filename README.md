...
念のため書いておくと、これは Firefox の方が安全という話ではありません。Chrome は 127 以降、Cookie の復号鍵をブラウザのバイナリに紐づけています（App-Bound Encryption）。マルウェア対策として設計されたもので、その結果として yt-dlp のような外部ツールも読めなくなりました。外部ツールから扱いやすいということは、防御が緩いということでもあります。

---

## 9. おまけ：Resonite 側のバグらしきもの

コマンドラインの先頭を見てください。

```
yt-dlp.exe --js-runtimes undefined ...
```

`undefined` という文字列がそのまま引数になっています。yt-dlp はこれを拒否します。

```
WARNING: Ignoring unsupported JavaScript runtime(s): undefined.
Supported runtimes: deno, node, bun, quickjs
```

Resonite の設定画面には JS ランタイムを指定する項目がないので、ユーザーの設定漏れではありません。

ただし実害は確認できませんでした。`yt-dlp.conf` に `--js-runtimes deno` を書いて比較実験もしましたが、画質にも再生成否にも影響しませんでした。

`RuntimeData` には `deno.EXE` が同梱されているので、`undefined` を無視したあとにこれが自動で使われているのではないかと考えていますが、**これは推測です。**Resonite 経由で確認するには `yt-dlp.conf` に `-v` を追加してログに `JS runtimes:` を出す必要があり、その状態では conf 自体が変数になるため、conf なしの状態を確かめる方法がありません。

YouTube 側が JavaScript チャレンジ必須の構成に変えたときには影響が出るかもしれません。
...
Resonite は映像と音声が1本になっているフォーマットしか選びません。音声つき HLS が提供されなければ、選べるのは itag 18（360p）だけになります。

その itag 18 を Cookie なしで取得すると `c=ANDROID_VR` の URL になり、`fexp` の値によっては 403 が返ります。Cookie ありだと `c=TVHTML5` になり、今のところ 403 は発生していません。ただしその代わりにライブが再生できなくなります。

Resonite が映像と音声を合成できるようになれば、この問題は発生しなくなります。
...
実際、7月28日・8月9日・8月15日の3時点で、最適な設定が2回入れ替わりました。この設定が正解、という書き方ができない対象なので、自分のログで確かめる方法を持っておくのが確実です。